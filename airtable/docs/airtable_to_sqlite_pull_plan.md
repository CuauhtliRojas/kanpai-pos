...
Esta fase trata Airtable como maestro de catálogos/configuración y conserva SQLite como runtime local-first. El pull es aditivo: crea o actualiza por clave natural, nunca borra registros ni relaciones y no modifica endpoints, modelos, migraciones o schema Airtable.
...
| 7 | Empleados | `empleados` / `Employee`; `roles_empleado` / `EmployeeRole` | `codigo_empleado` | nombre, alias, activo; roles solo aditivos | `pin_activo`, hash PIN y último acceso son seguridad/operación local; no se quitan roles ausentes |
...
| Empleados | roles | Roles / `clave_rol` (adición únicamente) |
...
- Estado operativo de mesa, sesiones, PIN, último acceso, tickets, pagos, impresión, inventario transaccional y SMS enviados.
...
- Las bajas en Airtable se representan con `activo=false`; eliminar registros remotos no elimina datos locales.
- Roles de empleado son append-only en esta fase. La reconciliación con bajas requiere una política explícita posterior.
