    ...
    <div className="border-4 border-[var(--kp-ink)] bg-[var(--kp-surface)] p-4 shadow-[var(--kp-shadow-hard-sm)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs font-bold text-[var(--kp-text-muted)]">{item.sku}</p>
        </div>
        <StatusChip status={item.stock_status} />
      </div>
      <div className="mt-3 flex items-end justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.1em] text-[var(--kp-text-muted)]">Stock</p>
          <p className="text-2xl font-black leading-none">
            {item.current_stock}{" "}
            <span className="text-sm font-bold text-[var(--kp-text-muted)]">{item.base_unit_name}</span>
          </p>
          <p className="mt-0.5 text-xs font-bold text-[var(--kp-text-muted)]">
        ...
        {onAdjust && item.active && (
          <button
            ...
            className="border-4 border-[var(--kp-ink)] bg-[var(--kp-surface-raised)] px-3 py-1.5 text-xs font-black uppercase shadow-[var(--kp-shadow-hard-sm)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none active:translate-x-[4px] active:translate-y-[4px] active:shadow-none"
          >
            Ajustar
          </button>
        )}
