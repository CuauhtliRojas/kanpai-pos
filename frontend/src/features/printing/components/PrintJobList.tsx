...
export function PrintJobList({ jobs, canReprint, activeJobId, isReprinting, errorMessage, onReprint }: Props) {
  if (jobs.length === 0) {
    return <div className="border-4 border-[var(--kp-ink)] bg-[var(--kp-surface)] p-8 text-center text-xl font-black uppercase shadow-[var(--kp-shadow-hard)]">Sin trabajos</div>;
  }
  return (
