...
export function TableCard({ table, selected, compact = false, onSelect }: TableCardProps) {
  return (
    <button
      type="button"
      ...
      className={[
        ...
        compact ? "min-h-28 gap-2 p-3" : "min-h-36 gap-3 p-4",
        selected
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <Armchair className={compact ? "h-5 w-5 shrink-0" : "h-7 w-7 shrink-0"} />
        <TableStatusBadge status={table.status} selected={selected} />
      </div>
      <div>
        <p className={`${compact ? "text-base" : "text-xl"} font-black uppercase leading-tight`}>
