...
const toneClassName: Record<StatusBadgeTone, string> = {
  ...
  neutral: "border-[var(--kp-ink)] bg-[var(--kp-surface-raised)] text-[var(--kp-ink)]",
...
export function StatusBadge({ label, tone = "neutral" }: StatusBadgeProps) {
  return (
    <span
      className={`inline-flex min-h-10 items-center border-4 px-3 text-xs font-black uppercase tracking-[0.08em] shadow-[var(--kp-shadow-hard-sm)] ${toneClassName[tone]}`}
    >
      {label}
    </span>
  );
